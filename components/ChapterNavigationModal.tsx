...
const ChapterNavigationModal: React.FC<ChapterNavigationModalProps> = ({ isOpen, onClose, chapters, currentChapterIndex, onSelectChapter }) => {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-4xl h-[90vh] bg-card rounded-lg shadow-xl flex flex-col"
        ...
        <div className="flex-grow p-4 sm:p-6 overflow-y-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {chapters.map((chapter, index) => (
              ...
              >
                <p className={`text-sm ${currentChapterIndex === index ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
                  {chapter.chapter === 0 ? 'Prefacio' : `Capítulo ${chapter.chapter}`}
                </p>
                <p className={`font-bold mt-1 ${currentChapterIndex === index ? 'text-primary-foreground' : 'text-foreground'}`}>
                  {chapter.title}
                </p>
              </button>
            ))}
